...
var RTable = function (props) {
    ...
    var data = props.data;
    var _b = react_1.useState([]), expandedRowKeys = _b[0], setExpandedRowKeys = _b[1];
    var _c = react_1.useState(5), columnsPageSize = _c[0], setColumnsPageSize = _c[1];
    var indentSize = react_1.useState(props.indentSize || 15)[0];
    var _d = react_1.useState('children' || props.childrenColumnName), childrenColumnName = _d[0], setChildrenColumnName = _d[1];
    var useFixedHeader = react_1.useState(props.useFixedHeader ? props.useFixedHeader : false)[0];
    ...
    var getThs = function () {
        ...
        return ths.map(function (c, index) {
            if (c.colSpan !== 0) {
                return react_1.default.createElement("th", { key: "" + c.key + index, colSpan: c.colSpan, className: c.className || '' }, c.title);
            }
        });
    };
