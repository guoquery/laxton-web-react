...
var RTableRow = function (props) {
    ...
    for (var i = 0; i < columns.length; i++) {
        ...
        if (render) {
            ...
            text = render({ text: text, record: record, index: index }) || {};
            // console.log(render, "render>>>>>>>", text)
            tdProps = text.props || {};
            if (typeof text !== "string" &&
                !react_1.default.isValidElement(text) &&
                "children" in text) {
                text = text.children;
            }
        ...
        indentText =
            i === 0 ? (react_1.default.createElement("span", { style: { paddingLeft: indentSize * indent + "px" }, className: prefixCls + "-indent indent-level-" + indent })) : null;
        if (!notRender) {
            // console.log('key', col)
            cells.push(react_1.default.createElement("td", { key: "" + col.key + index, colSpan: colSpan, rowSpan: rowSpan, className: "" + colClassName },
                indentText,
                expandIcon,
                text));
        }
